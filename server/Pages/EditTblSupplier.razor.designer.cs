...
namespace Localhost.Pages
{
    public partial class EditTblSupplierComponent : ComponentBase
    {
        ...
        {
            var growingGetTblSupplierBySupplierIdResult = await Growing.GetTblSupplierBySupplierId(SupplierId);
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Localhost.Models.Growing.TblSupplier args)
        {
            try
            {
                var growingUpdateTblSupplierResult = await Growing.UpdateTblSupplier(SupplierId, tblsupplier);
                DialogService.Close(tblsupplier);
            }
            catch (Exception growingUpdateTblSupplierException)
            {
                    NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to update TblSupplier");
            }
        }
