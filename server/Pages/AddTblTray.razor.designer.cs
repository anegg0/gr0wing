...
namespace Growing.Pages
{
    public partial class AddTblTrayComponent : ComponentBase
    {
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Growing.Models.Growing.TblTray args)
        {
            try
            {
                var growingCreateTblTrayResult = await Growing.CreateTblTray(tbltray);
                DialogService.Close(tbltray);
            }
            catch (Exception growingCreateTblTrayException)
            {
                    NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to create new TblTray!");
            }
        }
